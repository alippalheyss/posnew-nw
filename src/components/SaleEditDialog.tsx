...
const SaleEditDialog: React.FC<SaleEditDialogProps> = ({ isOpen, onClose, sale, onSave }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] flex flex-col font-faruma bg-card border-border text-foreground shadow-2xl" dir="rtl">
        ...
        <div className="flex-1 overflow-hidden grid grid-cols-1 md:grid-cols-2 gap-6 py-6">
           {/* Items Section */}
           <div className="flex flex-col bg-muted rounded-3xl border border-border overflow-hidden">
              <div className="p-4 border-b border-border flex items-center justify-between">
                 ...
                 <div className="relative w-40">
                    ...
                    <Input 
                      ...
                      className="h-8 text-[10px] bg-muted border-border pr-7 rounded-lg"
                    />
                    {productSearch && (
                       <div className="absolute top-full left-0 right-0 mt-1 bg-[#0f0f25] border border-border rounded-xl shadow-2xl z-50 overflow-hidden">
                       ...
                       <div key={item.id} className="flex items-center justify-between bg-black/20 p-3 rounded-2xl border border-border group">
                          ...
                          <div className="flex items-center gap-2 mr-4">
                             <div className="flex items-center gap-1 bg-muted rounded-lg p-0.5 border border-border">
                                <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-foreground" onClick={() => handleItemQtyChange(item.id, -1)}><Minus className="h-3 w-3" /></Button>
                                <span className="w-6 text-center text-[10px] font-black">{item.qty}</span>
                                <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-foreground" onClick={() => handleItemQtyChange(item.id, 1)}><Plus className="h-3 w-3" /></Button>
                             </div>
                             <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground/50 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all" onClick={() => handleRemoveItem(item.id)}><Trash2 className="h-4 w-4" /></Button>
                          </div>
                       </div>
                    ))}
                 </div>
              ...
              <div className="space-y-2">
                 ...
                 <div className="relative">
                    ...
                    <Select value={editedSale.customer?.id || 'walk-in'} onValueChange={handleCustomerChange}>
                       <SelectTrigger className="bg-muted border-border h-12 rounded-xl text-right pr-10 font-bold">
                    ...
                    {['cash', 'credit', 'card', 'mobile'].map(method => (
                       <Button
                          ...
                          className={cn(
                             ...
                             editedSale.paymentMethod === method ? "bg-primary text-foreground shadow-lg" : "bg-muted border-border text-muted-foreground hover:text-foreground"
                          )}
                       >
                          {method}
                       </Button>
                 ...
                 <div className="space-y-2">
                    ...
                    <div className="relative">
                       ...
                       <Input
                         ...
                         className="bg-muted border-primary h-14 rounded-2xl pr-14 text-2xl font-black text-foreground text-right"
                         placeholder="0.00"
                       />
                    </div>
                 </div>
