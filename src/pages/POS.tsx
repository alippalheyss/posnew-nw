  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-900 text-black dark:text-white font-faruma" dir="rtl">
      <div className="flex flex-1 flex-col lg:flex-row p-4 gap-4">
                        ...
                        return (
                          ...
                          >
                            <CardContent className="p-2 flex flex-col items-center text-center h-full justify-center">
                              ...
                              <p className={cn("font-bold text-[11px] truncate w-full text-black dark:text-white leading-tight", isLowStock && "text-red-700 dark:text-red-400")}>{product.name_dv}</p>
        ...
        <Card className="lg:w-1/3 flex flex-col h-[calc(100vh-120px)] overflow-hidden cart-container">
                    ...
                    <div key={item.id} className="flex items-center justify-between p-2 border rounded-md group hover:border-primary/50 transition-colors">
                      ...
                      <div className="flex-1 text-right mr-3">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="font-black text-lg text-black dark:text-white leading-tight">{item.name_dv}</p>
                            <p className="text-xs font-bold text-black dark:text-white mt-1 opacity-90">{item.name_en}</p>
                          </div>
                          {(item.units && item.units.length > 0) ? (
                            ...
                            <Badge variant="outline" className="text-[10px] py-0 h-5 bg-gray-100 text-black dark:text-white border-gray-200">
                              Piece
                            </Badge>
                          )}
                        </div>
                        ...
                        <div className="flex items-center justify-between mt-2 bg-gray-50 dark:bg-gray-800 p-1 rounded">
                          ...
                          <div className="flex items-center gap-3">
                            <div className="flex items-center border rounded bg-white dark:bg-black overflow-hidden h-7">
                              <Button variant="ghost" size="icon" className="h-full w-7 rounded-none border-l text-black dark:text-white hover:text-red-600" onClick={() => updateCartItemQty(item.id, -1)}>-</Button>
                              <Input
                                type="number"
                                value={item.qty}
                                onChange={(e) => {
                              ...
                              />
                              <Button variant="ghost" size="icon" className="h-full w-7 rounded-none border-r text-black dark:text-white hover:text-green-600" onClick={() => updateCartItemQty(item.id, 1)}>+</Button>
                            </div>
            ...
            <div className="flex-none p-4 border-t bg-gray-50 dark:bg-gray-900/50">
              ...
              <div className="mb-3 p-4 bg-gradient-to-r from-primary/10 to-primary/5 dark:from-primary/20 dark:to-primary/10 rounded-lg border-2 border-primary/30">
                <div className="text-center">
                  <div className="text-xs font-semibold text-black dark:text-white dark:text-black dark:text-white mb-1 uppercase tracking-wider">
            ...
            {creditDialogStep === 1 ? (
              <>
                <div className="px-6 relative">
                  <Search className="absolute right-9 top-1/2 -translate-y-1/2 h-4 w-4 text-black dark:text-white " />
                  <Input
              ...
              <div className="space-y-4 px-6 overflow-y-auto">
                ...
                <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg text-right">
                  <p className="text-sm text-black dark:text-white ">{renderBoth('customer')}</p>
                  <p className="font-bold text-lg">{activeCart?.customer?.name_dv}</p>
                  <p className="text-xs text-black dark:text-white ">{activeCart?.customer?.name_en} | {activeCart?.customer?.code}</p>
                </div>
        ...
        <DialogContent className="sm:max-w-[450px] font-faruma text-right p-0 overflow-hidden" dir="rtl">
          <div className="p-6">
            <DialogHeader className="pb-4 text-right">
              ...
              <DialogDescription className="text-black dark:text-white mt-2 break-words text-sm leading-relaxed text-right">
                {t('product_nearing_expiry', { name: selectedProductForExpiry?.name_dv })}
                <div className="mt-4 font-bold text-black dark:text-white border-t pt-4">
            ...
            {splitStep === 1 ? (
              <div className="space-y-4">
                <div className="relative">
                  <Search className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-black dark:text-white " />
                  <Input
                    placeholder={renderBothString('search_customers')}
                    value={customerSearchTerm}
                    onChange={(e) => setCustomerSearchTerm(e.target.value)}
                    ...
                    {splitEntries.map((entry) => {
                      ...
                      return (
                        <div key={entry.id} className="p-4 border rounded-lg bg-white dark:bg-gray-800">
                          <div className="flex justify-between items-center mb-2">
                            <span className="text-xs font-bold text-black dark:text-white ">Amount</span>
