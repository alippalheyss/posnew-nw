            ...
            <ScrollArea className="h-full">
              <div className="flex gap-3 p-4">
                {allSettlements.length === 0 ? (
                  <p className="text-black dark:text-white italic text-sm w-full text-center py-4">No recent settlements</p>
                ) : (
                  allSettlements.map((s) => (
                    <div key={s.id} className="min-w-[180px] bg-green-50 dark:bg-green-900/10 border border-green-100 rounded-lg p-3 text-right shadow-sm border-r-4 border-r-green-500">
                      ...
                      <p className="text-[10px] text-black dark:text-white mb-1">{s.date}</p>
              ...
              {filteredCustomers.map((customer) => (
                <Card key={customer.id} className="cursor-pointer hover:shadow-lg transition-shadow duration-200 text-right">
                  <CardContent className="p-4">
                    ...
                    <p className="text-sm text-black dark:text-white dark:text-black dark:text-white break-words">Code: {customer.code}</p>
                    ...
                    <p className="text-xs text-black dark:text-white dark:text-black dark:text-white break-words mb-2">Last Settlement: {customer.settlement_history.length > 0 ? customer.settlement_history[customer.settlement_history.length - 1].date : t('all_time')}</p>
        ...
        <DialogContent className="sm:max-w-[450px] font-faruma" dir="rtl">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="currentOutstanding" className="text-right block font-bold text-black dark:text-white ">
            ...
            <div className="space-y-2">
              <Label htmlFor="newOutstanding" className="text-right block font-bold text-black dark:text-white ">
          ...
          <div className="flex-1 overflow-hidden p-6 pt-0">
            <div className="h-full border rounded-lg overflow-hidden bg-white text-black shadow-inner">
              {selectedCustomerForAction?.settlement_history.length === 0 ? (
                <p className="text-center text-black dark:text-white py-20">{renderBoth('no_settlement_history')}</p>
              ) : (
                  ...
                  <div className="space-y-4">
                    {[...(selectedCustomerForAction?.settlement_history || [])].reverse().map((settlement) => (
                      <div key={settlement.id} className="border p-4 rounded-md bg-gray-50/50 flex flex-col gap-2">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-xs text-black dark:text-white ">
          ...
          <div className="flex-1 overflow-hidden p-6 pt-0">
            <div className="h-full border rounded-lg overflow-hidden bg-white text-black p-8 shadow-inner" id="credit-purchases-report-content">
              ...
              <div className="text-center mb-8 border-b pb-6">
                ...
                <p className="text-lg font-bold text-black dark:text-white mb-4">{settings.shop.shopName}</p>

                {settings.reports.showContactInfo && (
                  <div className="text-sm text-black dark:text-white flex flex-col gap-0.5">
              ...
              {selectedCustomerCreditSales.length === 0 ? (
                <p className="text-center text-black dark:text-white py-10">{renderBoth('no_credit_purchases')}</p>
              ) : (
                <ScrollArea className="h-[400px] pr-4">
                  <div className="space-y-6">
                    ...
                    {/* Report Footer */}
                    <div className="mt-12 text-center text-[10px] text-black dark:text-white italic py-6 border-t">
