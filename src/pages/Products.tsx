    ...
    <div className="p-6 font-faruma flex flex-col h-full bg-background text-foreground overflow-hidden" dir="rtl">
      ...
      <div className="flex flex-row-reverse gap-4 mb-4">
        <div className="relative flex-1">
           ...
           <Input 
             ...
             className="w-full bg-muted border-border rounded-xl pr-12 h-12 text-right font-bold focus:border-primary/50 transition-all"
           />
        </div>
        <div className="flex gap-2">
              ...
              {['DRINKS', 'FOOD', 'HARDWARE', 'COSMETICS', 'OTHER', 'ALL'].map((cat) => (
                <Button 
                  ...
                  className={cn(
                    ...
                    selectedCategory === cat ? "bg-primary text-foreground" : "text-muted-foreground hover:text-foreground"
                  )}
                >
                  {cat}
                </Button>
           ...
           <Button 
             ...
             className={cn(
               "h-12 w-12 p-0 rounded-xl border-border transition-all",
               showFavoritesOnly ? "bg-yellow-500 hover:bg-yellow-600 text-black shadow-[0_0_15px_rgba(234,179,8,0.3)]" : "hover:bg-muted"
             )}
           >
           ...
           <div className="bg-muted rounded-xl p-1 border border-border flex gap-1">
              <Button 
                ...
                className={cn("h-10 w-10 rounded-lg", viewMode === 'grid' ? "bg-muted/80 text-foreground" : "text-muted-foreground")}
              >
              ...
              <Button 
                ...
                className={cn("h-10 w-10 rounded-lg", viewMode === 'list' ? "bg-muted/80 text-foreground" : "text-muted-foreground")}
              >
      ...
      <ScrollArea className="flex-1 custom-scrollbar">
        {viewMode === 'grid' ? (
                      ...
                      <div className={cn(
                        ...
                        <div className="absolute top-2 left-2 right-2 flex justify-between items-start">
                           <Button 
                             ...
                             className={cn(
                               "h-7 w-7 rounded-lg backdrop-blur-md border border-border transition-all",
                               favoriteProductIds.includes(product.id) ? "bg-yellow-500 border-yellow-400 text-black shadow-lg" : "bg-black/20 text-foreground"
                             )}
                           >
                           ...
                           <DropdownMenu>
                             <DropdownMenuTrigger asChild>
                               <Button variant="ghost" size="icon" className="h-7 w-7 bg-black/20 text-foreground/70 hover:bg-black/40 backdrop-blur-md border border-border rounded-lg">
                                 <MoreVertical className="h-4 w-4" />
                               </Button>
                             </DropdownMenuTrigger>
                             <DropdownMenuContent className="bg-card border-border text-foreground" dir="rtl">
                               <DropdownMenuItem onClick={() => handleEditClick(product)} className="gap-2 text-right justify-end hover:bg-muted cursor-pointer">
          ...
          <Card className="bg-card border-border rounded-3xl overflow-hidden mb-6">
            <div className="overflow-x-auto">
              <table className="w-full text-right" dir="rtl">
                <thead>
                  <tr className="border-b border-border bg-muted">
                    <th className="p-4 text-[10px] font-black text-muted-foreground uppercase tracking-widest">Product</th>
                    <th className="p-4 text-[10px] font-black text-muted-foreground uppercase tracking-widest">Item Code</th>
                    <th className="p-4 text-[10px] font-black text-muted-foreground uppercase tracking-widest">Barcode</th>
                    <th className="p-4 text-[10px] font-black text-muted-foreground uppercase tracking-widest">Price</th>
                    <th className="p-4 text-[10px] font-black text-muted-foreground uppercase tracking-widest">Stock</th>
                    <th className="p-4 text-[10px] font-black text-muted-foreground uppercase tracking-widest">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {displayProducts.map((product) => (
        ...
        {visibleCount < filteredProducts.length && (
          <div className="flex justify-center py-8">
            <Button 
              ...
              variant="outline"
              className="bg-muted border-border hover:bg-muted/80 px-8 h-12 rounded-xl text-[10px] font-black uppercase tracking-widest"
            >
